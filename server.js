...
app.get("/catalog/search", async (req, reply) => {
  try {
    ...
    const qIsNumeric = /^\d+$/.test(qRaw);
    const qAsId = qIsNumeric ? Number(qRaw) : null;
    ...
    // search by:
    // - item title
    // - creator name / shop name
    // - creator type
    // - exact asset_id (if numeric)
    if (q.length > 0) {
      const qLikeIdx = params.length + 1;
      params.push(`%${q}%`);

      const terms = [
        `lower(coalesce(i.name,'')) LIKE $${qLikeIdx}`,
        `lower(coalesce(i.creator_name,'')) LIKE $${qLikeIdx}`,
        `lower(coalesce(i.creator_type,'')) LIKE $${qLikeIdx}`,
      ];

      if (qAsId !== null) {
        const qIdIdx = params.length + 1;
        params.push(qAsId);
        terms.push(`i.asset_id = $${qIdIdx}`);
      }
